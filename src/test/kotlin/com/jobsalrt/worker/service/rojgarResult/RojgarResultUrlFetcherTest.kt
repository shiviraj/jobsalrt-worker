...
class RojgarResultUrlFetcherTest {
    private val jobUrlService = mockk<JobUrlService>()
    private val webClientWrapper = mockk<WebClientWrapper>()
    private val rojgarResultUrlFetcher = RojgarResultUrlFetcher(webClientWrapper, jobUrlService)
    ...
    fun `should fetch all the urls`() {
        val jobUrl = JobUrlBuilder(name = "name", url = "http://rojgarresult.com/url").build()
        val jobUrl1 = JobUrlBuilder(name = "name1", url = "http://rojgarresult.com/url1").build()

        val htmlString = """<html>
                        <head></head>
                        <body>
                        <table><tr><td>
                            <ul><li><a href="http://rojgarresult.com/url">name</a></li></ul>
                            <ul><li><a href="http://rojgarresult.com/url1">name1</a></li></ul>
                        </td></tr></table>
                        </body>
                        </html>"""
        ...
        val fetch = rojgarResultUrlFetcher.fetch()
        assertNextWith(fetch, {
            it shouldBe jobUrl
        }, { it shouldBe jobUrl1 })
        verify(exactly = 1) {
            webClientWrapper.get(
                baseUrl = "https://www.rojgarresult.com",
                path = "/",
    ...
    fun `should get the urls from the html document`() {
        ...
        val jobUrls = rojgarResultUrlFetcher.getJobUrls(Jsoup.parse(htmlString))
        assertSoftly {
